...
nrep = 2
conds = ['DMSO_15min','DMSO_30min','DMSO_60min','NM_PP1_15min','NM_PP1_30min','NM_PP1_60min']
if endtype == '5prime':
...
def find_regions_to_keep(sampcts,samp):
  ...
  fivepct = int(len(colsums)/20)
  # Find middle 90% of regions
  keep_regs = colsums[fivepct:(len(colsums) - fivepct + 1)]
...
def bin_and_calculate(ctdf,bsize):
  ...
  for j in range(0,(window-bsize-1)):
    bsum = ctdf.iloc[j:(j+bsize+1),:].sum(axis=0)
